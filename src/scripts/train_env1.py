...
import datetime
import csv
from collections import deque  # [新增] 用于滑动窗口
...
MAX_EPISODES = 500
MAX_TRAIN_STEPS = 999999999
MAX_EPISODE_STEPS = 50

# [自定义] 成功率统计窗口大小
SUCCESS_WINDOW_SIZE = 10 

CHECKPOINT_STEPS = [1000, 2000, 4000, 6000, 8000, 10000, 15000, 20000, 25000, 30000, 40000, 50000, 75000, 100000]
...
def main() -> None:
    ...
    for i_episode in pbar:
        ...
        while not (done or truncated):
            ...
            if replay_buffer.size() > cfg.minimal_size:
                ...
                current_loss = float(loss_info.get("q_loss", 0.0) + loss_info.get("fuzzy_loss", 0.0)) if isinstance(loss_info, dict) else float(loss_info)
            ...
            if total_steps in CHECKPOINT_STEPS:
                ...
                tqdm.write(f">>> [Checkpoint] Model saved: {os.path.basename(save_path)} at step {total_steps}")

            if total_steps % 10 == 0:
                elapsed_time = time.time() - start_time
                steps_per_sec = total_steps / (elapsed_time + 1e-9)
                pbar.set_postfix({"T_Steps": total_steps, "S/s": f"{steps_per_sec:.1f}"})
    ...
    agent.save(os.path.join(MODEL_DIR, f"{ALGO_NAME}_{TIMESTAMP}_final.pth"))
    env.close()
    csv_file.close()
    writer.close()
